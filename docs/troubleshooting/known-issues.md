...
| **A DOCX parse cannot be interrupted once it starts.** | A crafted DOCX could occupy the parser for as long as it takes to finish. | Bounded rather than closed. Uploads are capped at 64 MB each and 64 MB per message, and DOCX extraction carries the same 60-second ceiling as PDF — but a timeout cannot interrupt synchronous work, so the size cap is what actually holds. Only ingest documents you trust. |
