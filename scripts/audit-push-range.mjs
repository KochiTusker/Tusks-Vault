 ...
 * scope, which this project's own threat model does not: dev is second-line
...
for (const line of refLines) {
  ...
  try {
    ...
    // resolvePrivateNames throws on the public path when the list file is
    // missing — an unscanned push must never read as a safe one.
