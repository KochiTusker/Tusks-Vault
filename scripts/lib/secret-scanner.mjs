...
//   private-name  — a local string list      (private-names.mjs)
...
const CREDENTIAL_FILENAME_RES = [
  ...
  { re: /(^|\/)api-keys\.json$/, why: "Vault's key store — runtime state, never tracked" },
  { re: /(^|\/)settings\.json$/, why: "Vault's settings — runtime state, never tracked", allow: /(^|\/)\.(vscode|claude)\// },
  { re: /(^|\/)\.private-names$/, why: "a local-only list file — publishing it defeats its purpose" },
];
...
/**
 * Blank out the anchor slugs a static-site generator derives from headings,
 * before any credential-shape matching runs over built HTML.
 *
 * A heading like "### Rotate the sk-ant key on a schedule" becomes
 * id="rotate-the-sk-ant-key-on-a-schedule", and a long enough kebab run
 * satisfies the "opaque high-entropy string" shape several token patterns
 * look for. Those hits are all false: the slug is derived from prose that is
 * already in the page, so there is nothing in it that was not published
 * deliberately.
 *
 * It costs no coverage. Only the VALUE of an id/href anchor is removed, and
 * only when that value is a pure lowercase-kebab slug — the exact shape a
 * generator emits and the one shape no real credential has (they carry case,
 * digits in runs, and punctuation outside [a-z0-9-]). A secret pasted into a
 * heading survives in the heading's own text, which the scanner still reads.
 */
export function stripGeneratedSlugs(content) {
  return String(content)
    .replace(/\bid="[a-z0-9]+(?:-[a-z0-9]+)+"/g, 'id=""')
    .replace(/\bhref="#[a-z0-9]+(?:-[a-z0-9]+)+"/g, 'href="#"');
}
