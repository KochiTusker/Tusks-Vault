// String denylist — checks tracked files against a list of literal strings
// that should not appear in them.
//
// The list is not in the repo. Every other layer can keep its patterns in
// tracked source, because a regex for an API key is not itself an API key.
// That does not hold for a literal denylist: it has to contain the exact
// strings it checks for, so a committed one would publish its own contents.
// It lives in `.private-names`, which is gitignored; `.private-names.example`
// documents the format for anyone cloning the repo.
//
// FAILURE MODE, AND THE ASYMMETRY THAT HANDLES IT
// ================================================
// A denylist that silently no-ops when its data file is missing is worse than
// no denylist, because it still reads as protection. But the file is
// legitimately absent for contributors, who must be able to build and test:
//
//   - Tree audit / dev-side runs: absent list → loud warning, not a failure.
//   - Push to the public remote: absent list → HARD FAILURE, because that is
//     the last point at which the check still means anything.
//
