...
describe("disclosure layer", () => {
  ...
  // Regression: every one of these shipped in v0.1.0 and none of them fired.
  // They are kept as literal strings rather than paraphrases, because the
  // near-misses are the point — see the reversed word order below.
  it("flags the comment shapes that reached the first public release", () => {
    const f = "src/server/example.ts";
    const shipped = [
      ...
      "// dir name, where an unanchored entry once hid real source from git.",
      "// from the opsec scanner. Same class of defect both times: an escape",
    ];
    for (const line of shipped) {
      expect(scanLinesForDisclosure(f, line), line).toHaveLength(1);
    }
  });

  it("flags announced concealment in either word order", () => {
    // The original rule only matched "so ... timezone ... reveal". The
    // sentence that actually shipped put the verb first, and was invisible.
    expect(
      scanLinesForDisclosure("a.md", "dates are UTC so the timezone cannot reveal a location")
    ).toHaveLength(1);
    expect(
      scanLinesForDisclosure("a.md", "normalised so a commit doesn't leak the author's local timezone")
    ).toHaveLength(1);
  });

  it("reads prose, not code — an import path is not a disclosure", () => {
    // A first cut scanned every line and produced 26 findings, ~2 of them
    // real; the noise was module paths and ordinary vocabulary. A layer that
    // cries wolf gets switched off, so code lines are skipped in code files.
    const imp = 'import { resolvePrivateNames } from "./lib/private-names.mjs";';
    expect(scanLinesForDisclosure("scripts/thing.mjs", imp)).toHaveLength(0);
    expect(scanLinesForDisclosure("scripts/thing.mjs", "// see private-names for the list")).toHaveLength(1);
    // ...but Markdown is prose all the way down.
    expect(scanLinesForDisclosure("docs/x.md", "the private-names list holds them")).toHaveLength(1);
  });

  it("treats allowlist/denylist as ordinary vocabulary unless it is about people", () => {
    expect(
      scanLinesForDisclosure("a.mjs", "// An allowlist rather than a denylist, because the failure mode")
    ).toHaveLength(0);
    expect(
      scanLinesForDisclosure("a.mjs", "// the denylist of maintainer names lives outside the repo")
    ).toHaveLength(1);
  });
});
...
describe("stripGeneratedSlugs", () => {
  it("blanks the anchor a generator derives from a heading", () => {
    const html = '<h3 id="rotate-the-key-on-a-schedule">Rotate the key</h3>';
    expect(stripGeneratedSlugs(html)).toBe('<h3 id="">Rotate the key</h3>');
  });

  it("blanks the matching table-of-contents link", () => {
    expect(stripGeneratedSlugs('<a href="#rotate-the-key-on-a-schedule">')).toBe('<a href="#">');
  });

  it("leaves single-word ids alone — they cannot reach a token shape", () => {
    expect(stripGeneratedSlugs('<div id="main">')).toBe('<div id="main">');
  });

  it("does not touch prose, only anchor values", () => {
    // The whole justification for the suppression: a secret pasted into a
    // heading still reaches the scanner through the heading's own text.
    const html = '<h3 id="a-b-c">sk-ant-api03-DEADBEEFdeadbeef0123456789</h3>';
    expect(stripGeneratedSlugs(html)).toContain("sk-ant-api03-DEADBEEFdeadbeef0123456789");
  });

  it("leaves a credential-shaped id alone, because it is not slug-shaped", () => {
    // Uppercase and underscores are outside the generated-slug charset, so a
    // real token sitting in an id attribute is still scanned.
    const html = '<div id="AKIAIOSFODNN7EXAMPLE">';
    expect(stripGeneratedSlugs(html)).toBe(html);
  });
});
