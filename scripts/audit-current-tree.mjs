...
// ── Layer: string denylist availability ────────────────────────────
...
for (const file of files) {
  ...
  if (buf.includes(0)) {
    ...
    //
    // A source file whose regex character class holds raw control bytes
    // where the escape sequences belong will compile, pass its tests, and
    // still be routed past every content layer above. Hence this branch.
